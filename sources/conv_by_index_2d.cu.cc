...
__device__ void singlePointConv(float* inpt, const float *filter, float *out, size_t filIndex, size_t in_channels, size_t in_cols, size_t filter_rows,
    ...
    for(size_t r=0; r < filter_rows; r++){
        const float *frp = filter + r*frStep;
        float *irp = inpt+r*irStep;
        for(size_t c=0; c < filter_cols; c++){
            const float *fcp = frp + c*fcStep;
            for(size_t d=0; d < in_channels; d++) result += (*irp++)*fcp[d*out_channels];
        }
    }
    *out = result;
}
...
template <typename T>
__global__ void ConvByIndexCudaKernel(int index, T *input, T *out, const T *filters, const int *ind, const int *msk, size_t outbStep, size_t outrStep, size_t out_depth,
  ...
  size_t paddingOffsetRows, size_t paddingOffsetCols, size_t numJobs){
      ...
      for(size_t outIndex=(size_t)threadID; outIndex<numJobs;outIndex+=num_threads){
          if(ind[outIndex] != index) continue;
          ...
          T *inpt = input + n*inbStep + r*irStep + c*in_depth;
          // const T *fltr = filters + (size_t)(filSetSize*ind[outIndex]);
          if(msk[outIndex]) singlePointConv(inpt, filters+f, out+outIndex, f, in_depth, in_cols, filter_rows, filter_cols, out_depth, frStep, irStep, fcStep);
          else *(out+outIndex) = 0;
      }
  }
...
struct ConvByIndexFanctor<GPUDevice, T>{
  ...
  for(int index = 0; index<num_filter_sets; index++){
    cudaDeviceSynchronize();
    ConvByIndexCudaKernel<T><<<cfg.block_count, cfg.thread_per_block, 0, context->eigen_device<GPUDevice>().stream()>>>
    (index, newIn, out, fil+(size_t)(index*filSetSize), ind, msk, (size_t)outbStep, (size_t)outrStep, (size_t)out_depth, (size_t)inbStep, (size_t)irStep, (size_t)in_depth, (size_t)filSetSize,
